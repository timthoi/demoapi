...
* @subpackage	Form
*/
class JFormRuleNumeric extends GolidaysClassFormRule
{
	/**
	...
	*/
	public function getJsonRule($field)
	{
		...
		else if ($numericFormat = $field->getAttribute('numericFormat'))
		{
			...
			$alertText = JText::sprintf('GOLIDAYS_ERROR_INCORRECT_FORMAT_EXPECTED_DIGITS_MAX_DECIMALS_MAX', $length, $scale);
		}
		else
			// Fallback default error message for all numeric types
			$alertText = JText::sprintf('GOLIDAYS_ERROR_EXPECTED_NUMERIC_FORMAT');
