...
class JFormFieldUpload extends JFormField
{
	...
	*/
	protected function getInputMaxsize()
	{
		...
		$maxsize = GolidaysHelperFile::getUploadMaxSize(true, $uploadMaxsize);
	...
	*/
	protected function getInputThumb()
	{
		...
		if (!empty($this->value))
		{
			...
			$previewUrlSrc = GolidaysHelperFile::getFileUrlFromElement($this->form, $this->element, $this->value);
	...
	*/
	protected function uploadActionsList()
	{
		...
		$list[] = array('value' => '', 'text' => JText::_("GOLIDAYS_UPLOAD_REMOVE_KEEP"), 'icon' => 'cancel');

		if (in_array('remove', $actions))
			$list[] = array('value' => 'remove', 'text' => JText::_("GOLIDAYS_UPLOAD_REMOVE_EJECT"), 'icon' => 'out');

		if (in_array('thumbs', $actions))
			$list[] = array('value' => 'thumbs', 'text' => JText::_("GOLIDAYS_UPLOAD_REMOVE_THUMBS_ONLY"), 'icon' => 'pictures');

		if (in_array('trash', $actions))
			$list[] = array('value' => 'trash', 'text' => JText::_("GOLIDAYS_UPLOAD_REMOVE_TRASH"), 'icon' => 'trash');

		if (in_array('delete', $actions))
			$list[] = array('value' => 'delete', 'text' => JText::_("GOLIDAYS_UPLOAD_REMOVE_DELETE"), 'icon' => 'remove');
