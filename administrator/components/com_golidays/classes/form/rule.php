...
/**
* Form validator rule for Golidays.
*
* @package	Golidays
* @subpackage	Form
*/
class GolidaysClassFormRule extends JFormRule
{
	/**
	...
	*/
	public function getJsonRule($field)
	{
		...
		*/
		...
		$json = GolidaysHelperHtmlValidator::jsonFromArray($values);
		return "{" . LN . $json . LN . "}";
	}
	...
	*/
	public static function testDefaults(&$element, $value, $group = null, &$input = null, &$form = null)
	{
		...
		if ($unique)
		{
			...
			if ($duplicate)
			{
				JError::raiseWarning(1201, JText::sprintf("GOLIDAYS_MODEL_DUPLICATE_ENTRY_FOR", JText::_($element['label'])));
				return false;
			}
		}
		...
		if (!empty($uniqueGroups))
		{
			...
			foreach($uniqueGroups as $alias => $uniqueGroup)
			{
				...
				if ($duplicate)
				{
					JError::raiseWarning(1201, JText::sprintf("GOLIDAYS_MODEL_DUPLICATE_ENTRY_FOR", implode (', ', $fieldLabels)));
					return false;
				}
			}
		}
