...
* @subpackage	Classes
*/
class GolidaysClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'GolidaysModel');
	...
	public function getItems()
	{
		try
		{
			...
			if ($error = $db->getErrorMsg()) {
				if (!$this->canAdmin())
					$error = JText::_('GOLIDAYS_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'GolidaysModel');
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_golidays', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = GolidaysHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'GolidaysModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__golidays_' . $this->getName())
