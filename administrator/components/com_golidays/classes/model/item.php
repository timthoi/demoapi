...
* @subpackage	Classes
*/
class GolidaysClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("GOLIDAYS_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_golidays' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!GolidaysHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public static function fkTable($fieldname)
	{
		...
		switch($fieldname)
		{
			case 'continent': return $tbl.$com. 'continents';
			case 'departure_city_id': return $tbl.$com. 'departurecities';
			case 'continent_id': return $tbl.$com. 'continents';
			...
			case 'ticket_type_id': return $tbl.$com. 'tickettypes';	
		}
	}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'GolidaysModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_GOLIDAYS . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'GolidaysModel');
		if ($modelList)
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_GOLIDAYS . '/models/forms');
		JForm::addFieldPath(JPATH_GOLIDAYS . '/models/fields');
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'golidaysModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Golidays"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_GOLIDAYS . '/fork/models/forms';
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("GOLIDAYS_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format)
					{
						...
						if (!is_numeric($value) && !GolidaysHelperDates::isNull((string)$value))
						{
							$time = GolidaysHelperDates::getSqlDate($value, array($field->format));

							if ($time === null){
								JError::raiseWarning(1203, JText::sprintf('GOLIDAYS_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
								$valid = false;
							}
							else
