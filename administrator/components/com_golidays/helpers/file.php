...
defined('GOLIDAYS_UPLOAD_DEFAULT_DIR') or define("GOLIDAYS_UPLOAD_DEFAULT_DIR", 'COM_MEDIAS');
defined('GOLIDAYS_UPLOAD_DEFAULT_RENAME') or define("GOLIDAYS_UPLOAD_DEFAULT_RENAME", '{ALIAS}.{MIMEXT}');
defined('GOLIDAYS_UPLOAD_CHMOD_FOLDER') or define("GOLIDAYS_UPLOAD_CHMOD_FOLDER", 0754);
defined('GOLIDAYS_UPLOAD_CHMOD_FILE') or define("GOLIDAYS_UPLOAD_CHMOD_FILE", 0644);
defined('GOLIDAYS_IMAGES_MAX_WIDTH') or define("GOLIDAYS_IMAGES_MAX_WIDTH", 1000);
defined('GOLIDAYS_IMAGES_MAX_HEIGHT') or define("GOLIDAYS_IMAGES_MAX_HEIGHT", 1000);
defined('GOLIDAYS_IMAGES_ALLOWED_SIZES') or define("GOLIDAYS_IMAGES_ALLOWED_SIZES", '');
...
class GolidaysHelperFile
{
	...
	* @var string
	*/
	protected static $extension = 'com_golidays';
		...
		if (static::checkFileExists($dir, $file))
		{
			switch(strtolower($options["overwrite"]))
			{
				...
				case '0':
					...
					throw new \Exception(JText::sprintf( "GOLIDAYS_UPLOAD_EXISTS", $file->filename));
					break;
	...
	public static function getFileUrlFromElement($form, $element, $value)
	{
		...
		if (!$dir)
		{
			...
			if (!in_array($dir, array_keys($directories)))
				$dir = GOLIDAYS_UPLOAD_DEFAULT_DIR;
		...
		$attribs = GolidaysHelperFile::getAttributesFromElement($element);

		$url = (($method == 'indirect') && JFactory::getApplication()->isAdmin()?'administrator/':'')
			. GolidaysHelperFile::getFileUrl($indirectPath, $method, $attribs);
	...
	*/
	public static function getUploadMaxSize($string = false, $maxSizeCustom = null)
	{
		...
		if ($string)
			$maxSize = JText::sprintf("GOLIDAYS_UPLOAD_MAX", static::bytesToString($maxSize));
		...
		if (JFile::exists($filePath))
		{
			...
			if (in_array($remove, array('trash')))
			{
				...
				if (!JFile::move($filePath, $trashPath .'/'. JFile::getName($filePath)))
					throw new \Exception(JText::_("GOLIDAYS_UPLOAD_TRASH_IMPOSSIBLE"));
			...
			else if (in_array($remove, array('delete')))
			{
				if (!JFile::delete($filePath))
					throw new \Exception(JText::_("GOLIDAYS_UPLOAD_DELETE_IMPOSSIBLE"));
	...
	*/
	...
	{
		$pattern = (isset($options["rename"])?$options["rename"]:GOLIDAYS_UPLOAD_DEFAULT_RENAME);
	...
	*/
	public static function returnFile()
	{
			...
			)
		{
			...
			{
				$w = (int)min($size->width, GOLIDAYS_IMAGES_MAX_WIDTH);
				$h = (int)min($size->height, GOLIDAYS_IMAGES_MAX_HEIGHT);
				...
				if (($size != "0x0") && !in_array($size, explode(",", GOLIDAYS_IMAGES_ALLOWED_SIZES)))
				{
		...
		else if (!JFile::exists($filePath) || in_array($ext, $forbiddenExt))
		{
			...
			$msg = JText::sprintf( "GOLIDAYS_UPLOAD_FILE_NOT_FOUND", $pathNoDir);
			jexit($msg);
		}
	...
	*/
	public static function sanitizeFolder($path)
	{
		...
		@chmod($folder, GOLIDAYS_UPLOAD_CHMOD_FOLDER);
		if (!is_writable($folder))
			throw new \Exception(JText::sprintf( "GOLIDAYS_UPLOAD_READONLY_FOLDER",$path));
	...
	*/
	public static function uploadFile($dir, $uploadFile, $options = array())
	{
		...
		if (!$dir)
			throw new \Exception(JText::_("GOLIDAYS_UPLOAD_FOLDER_NOT_FOUND"));
		...
		if (empty($uploadFile['name']))
			throw new \Exception(JText::_("GOLIDAYS_UPLOAD_PLEASE_BROWSE"));
		...
		{
			throw new \Exception(JText::sprintf( "GOLIDAYS_UPLOAD_REFUSED_EXTENSION",
				$file->extension,
		...
		{
			throw new \Exception(JText::sprintf( "GOLIDAYS_UPLOAD_REFUSED_MIME",
				$file->mime,
				implode(" - ", $extensions))
			);
		}
		...
		{
			throw new \Exception(JText::sprintf( "GOLIDAYS_UPLOAD_REFUSED_SIZE",
		...
		if (preg_match("/\<\?php\s/", $contents))
			throw new \Exception(JText::_( "GOLIDAYS_UPLOAD_CONTAINS_ERRORS"));
	...
	protected static function uploadFileProcess($dirBase, $filePath, $fileTmp)
	{
		...
		if (!move_uploaded_file($fileTmp, $fileDest))
			if (!JFile::upload($fileTmp, $fileDest))
			{
				...
				throw new \Exception(JText::sprintf( "GOLIDAYS_UPLOAD_ERROR_UPLOAD", $file->filename));
			}


		//Protect file against execution
		@chmod($fileDest, GOLIDAYS_UPLOAD_CHMOD_FILE);
