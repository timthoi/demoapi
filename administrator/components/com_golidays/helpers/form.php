...
class GolidaysHelperForm
{
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = GolidaysHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'golidays.') . $relation->model;

				$model = GolidaysHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = GolidaysClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return GolidaysHelper::componentModel($model, $item);
	}
